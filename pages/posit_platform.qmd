...
HRL data scientists work primarily in R and Python and are familiar with RStudio, Positron, JupyterLab, and Visual Studio Code. The Azure stack and pipelines described on the [Data Pipeline Architecture](spatial_data_pipeline_architecture.qmd) page handle data ingestion, validation, standardization, storage, and public-facing APIs — but those services (Container Apps, API Management, App Service, etc.) are designed for infrastructure engineers, not ecologists running analyses and building Shiny applications. A data scientist should not need to understand Azure deployment mechanics to share a dashboard with a colleague.
...
::: {.callout-note}
This page describes the data science tooling layer. It complements the data engineering layer described on the [Data Pipeline Architecture](spatial_data_pipeline_architecture.qmd) page; it does not replace it. Both are needed.
:::
...
In practice, the two may coexist: infrastructure-managed applications and public APIs can run on Azure, while R/Python analytical applications, reports, and controlled internal tools can run on Posit Connect, depending on audience, deployment needs, and developer workflow.
...
- [ADR-011: Procure and use Posit Workbench and Posit Connect for data science and application publishing](../adr/adr/adr-011-posit-workbench-posit-connect.md)
- [ADR-012: Separate the Azure data engineering stack from the Posit data science platform](../adr/adr/adr-012-separate-azure-data-engineering-and-posit.md)
- [ADR-013: Treat Posit as a deliberate IaaS exception to the PaaS-first principle](../adr/adr/adr-013-posit-iaas-exception.md)
